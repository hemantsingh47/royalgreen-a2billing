<?php
include(dirname(__FILE__).'/../includes/methods/paystack.php');

class paystack
{
    public $code, $title, $description, $enabled, $mode;
    public $paystack_allowed_currencies = array('CAD', 'EUR', 'GBP', 'JPY', 'USD', 'MXN', 'AUD', 'NZD', 'BRL');
    ...
    public function paystack()
    {
        ...
        $this->code = 'paystack';
        $this->title = 'Paystack';
        $this->description = MODULE_PAYMENT_PAYSTACK_TEXT_DESCRIPTION;
        $this->sort_order = 1;
        $this->enabled = ((MODULE_PAYMENT_PAYSTACK_STATUS == 'True') ? true : false);
        $this->mode = ((PAYSTACK_PAYMENT_MODE == 'TEST') ? test : live);
	//$this->enabled = true;

        $this->form_action_url = "https://billing.adoreinfotech.co.in/crm/customer/paystack_pay.php";
    }
    ...
    public function update_status()
    {
        ...
        if ( ($this->enabled == true) && ((int) MODULE_PAYMENT_PAYSTACK_ZONE > 0) ) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_PAYSTACK_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    public function process_button($transactionID = 0, $key= "", $type = '_WEB')
    {
        ...
        if (!in_array($my_currency, $this->stripe_allowed_currencies)) {
            $my_currency = 'USD';
        }
        $currencyObject = new currencies();
        $process_button_string = tep_draw_hidden_field('cmd', '_xclick') .
                               tep_draw_hidden_field('business', MODULE_PAYMENT_PAYSTACK_ID) .
	...
	public function process_button_mo($transactionID = 0, $key= "")
    {
        ...
        if (!in_array($my_currency, $this->paystack_allowed_currencies)) {
            $my_currency = 'USD';
        }
        $currencyObject = new currencies();
        $process_button_string = tep_draw_hidden_field('cmd', '_xclick') .
                               ...
                               tep_draw_hidden_field('currency_code', $my_currency) .
                               //tep_draw_hidden_field('notify_url', tep_href_link("checkout_process.php?transactionID=".$transactionID."&sess_id=".session_id()."&key=".$key, '', 'SSL')) .
                               tep_draw_hidden_field('return', tep_href_link("cancel_payment.php", '', 'SSL')) .
    ...
    {
        $my_currency = MODULE_PAYMENT_PAYSTACK_CURRENCY;
        $base_currency = strtoupper($GLOBALS['A2B']->config['global']['base_currency']);
        if ($my_currency =='Selected Currency' && in_array($base_currency, $this->paystack_allowed_currencies) ) {
            $my_currency = $base_currency;
        } elseif (!in_array($my_currency, $this->paystack_allowed_currencies)) {
            $my_currency = 'NGN';
        }
    ...
    {
        return array('MODULE_PAYMENT_PAYSTACK_STATUS', 'MODULE_PAYMENT_PAYSTACK_ID', 'PAYSTACK_PAYMENT_MODE');
    }
}
