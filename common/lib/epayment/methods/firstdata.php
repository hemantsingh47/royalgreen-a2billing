<?php  
include(dirname(__FILE__).'/../includes/methods/firstdata.php');   
 
 $DBHandle  = DbConnect();   
 $username = $_SESSION['card_id'];
 $inst_table = new Table();
 
  $query = "SELECT * FROM cc_card WHERE id = '".$username."'";
  $customer_currency = $inst_table -> SQLExec($DBHandle, $query);
  //print_r($customer_currency[0]['currency']);  

class firstdata
{
    public $code, $title, $description, $enabled;
    public $paypal_allowed_currencies = array('AED','CAD', 'EUR', 'GBP', 'JPY', 'USD', 'MXN', 'AUD', 'NZD', 'BRL');
    ...
    public function firstdata()
    {
        ...
        $this->form_action_url = FIRSTDATA_PAYMENT_URL;
    }
