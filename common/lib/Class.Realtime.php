...
class Realtime
{
    ...
    public function create_trunk_config_file ($type = 'sip')
    {
        ...
        if ($type == 'iax') {
            ...
            $buddyfile = BUDDY_SIP_FILE;
             echo '<script>alert("hemant '.BUDDY_SIP_FILE.'");</script>';exit();
    ...
    //new update function
     public function update_voip_config ($sip, $iax, $id_card, $accountnumber, $passui_secret)
    {
        $who_id = '';
        if (!isset ($sip))
            $sip = 0;
        ...
        if ($sip) {
            ...
            $FG_QUERY_ADITION_SIP_FIELDS_WITH_VAL = "name='$accountnumber', accountcode='$accountnumber', regexten='$accountnumber' , username='$accountnumber' , secret='$passui_secret'";
            $instance_sip_table = new Table($FG_TABLE_SIP_NAME, "*");
            $clause="id_cc_card='$id_card'";
            $result_query1 = $instance_sip_table->Update_table($this->DBHandler, $FG_QUERY_ADITION_SIP_FIELDS_WITH_VAL, $clause);
            if (USE_REALTIME) {
                $_SESSION["is_sip_iax_change"] = 1;
                $_SESSION["is_sip_changed"] = 1;
            }
        ...
        if ($iax) {
            ...
            $FG_QUERY_ADITION_IAX_FIELDS_WITH_VAL = "name='$accountnumber', accountcode='$accountnumber', regexten='$accountnumber' , username='$accountnumber' , secret='$passui_secret'";
            $instance_iax_table = new Table($FG_TABLE_IAX_NAME, "*");
             $clause="id_cc_card='$id_card'";
            $result_query2 = $instance_iax_table->Update_table($this->DBHandler, $FG_QUERY_ADITION_IAX_FIELDS_WITH_VAL,$clause);
            if (USE_REALTIME) {
                $_SESSION["is_sip_iax_change"] = 1;
                $_SESSION["is_iax_changed"] = 1;
            }
