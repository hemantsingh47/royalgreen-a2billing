...
if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['phone_user'] && $_POST['otp'] && $_POST['ccode']) 
{
    ...
    //$username = $ccode . $username;
    $query_card = "SELECT username,firstname,lastname,uipass,email,phone,ccode FROM cc_card WHERE username='" .
        $username . "'";
    ...
    if ($total_available_dids <= 0) {
        ...
        if ($result_otp_info) {
            ...
            if ($result_card_info) {
                $otp_send = true;
                $userinfo = array('username' => $result_card_info[0]['username'], 'password' =>
                        $result_card_info[0]['uipass']);
                ...
                $id_cc_card = $instance_sub_table->Add_table($DBHandle, "NOW(),'$idDID','$idDID',$idDID, $uipassword, '$username','$tarff_id','$initial_amount','BDT','$ccode'", null, null,
                    'id');
				//print_r($instance_sub_table);
