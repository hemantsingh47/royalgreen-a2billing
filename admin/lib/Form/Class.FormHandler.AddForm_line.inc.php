...
<script language="JavaScript" src="./javascript/calonlydays.js"></script>
    ...
    <FORM action=<?php echo filter_input(INPUT_SERVER, 'PHP_SELF', FILTER_SANITIZE_URL); ?> id="myForm" method="post" class="kt-form" name="myForm">
		<div class="kt-portlet__body">
			<div class="form-group">
				<TABLE  class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline" style="width:100%;" align="center">
								...
								<?php
									...
									for ($i=0;$i<$this->FG_NB_TABLE_ADITION;$i++) 
									{
										...
										if (strlen($this->FG_TABLE_ADITION[$i][16])>1 && strtoupper ($this->FG_TABLE_ADITION[$i][3])!=("HAS_MANY")) 
										{
											 ...
											 echo '<br><strong style="font-size:18px"><i>';
											 echo $this->FG_TABLE_EDITION[$i][16];
											 echo '</i></strong><hr style="background:#000;height:1px"/>';
								...
								<div class="uk-width-large-1-2"> 
									...
									?>
											...
											<div class="uk-form-row">  
												...
												<label for="<?php echo $this->FG_TABLE_ADITION[$i][1]?>" 
										...
										<?php
											if ($this->FG_DEBUG == 1) 
												print($this->FG_TABLE_ADITION[$i][3]);
											if (strtoupper ($this->FG_TABLE_ADITION[$i][3])=="INPUT") 
												{
										?>             
										<INPUT class="md-input" name=<?php echo $this->FG_TABLE_ADITION[$i][1]?>  <?php echo $this->FG_TABLE_ADITION[$i][4]?> value="<?php echo $processed[$this->FG_TABLE_ADITION[$i][1]];?>">
									...
									?>
										<table cellpadding="2" cellspacing="0" border="0" width="100%">
											...
											<INPUT class="form-control" name=<?php echo $this->FG_TABLE_ADITION[$i][1]?>  <?php echo $this->FG_TABLE_ADITION[$i][4]?> value="<?php echo $processed[$this->FG_TABLE_ADITION[$i][1]];?>"><span class="md-input-bar "></span></div> Enter code from above picture here.
											</td>
											</tr>
											</table> 
...
<!--CAPTCHA IMAGE CODE END HERE-->
    ...
    ?>
        ...
        <INPUT class="form-control" name=<?php echo $this->FG_TABLE_ADITION[$i][1]?>  <?php echo $this->FG_TABLE_ADITION[$i][4]?> value="<?php if ($this->VALID_SQL_REG_EXP) { echo stripslashes($list[0][$i]); } else { echo $processed[$this->FG_TABLE_ADITION[$i][1]]; }?>"><span class="md-input-bar "></span></div>
    ...
    ?>
            ...
            <TEXTAREA class="form-control" style="overflow-x: hidden; word-wrap: break-word;height:120px" name=<?php echo $this->FG_TABLE_ADITION[$i][1]?> <?php echo $this->FG_TABLE_ADITION[$i][4]?>><?php echo $processed[$this->FG_TABLE_ADITION[$i][1]];?></TEXTAREA>  </div>
    ...
    ?>
           ...
           <SELECT  class="form-control" name='<?php echo $this->FG_TABLE_ADITION[$i][1]?><?php if (strpos($this->FG_TABLE_ADITION[$i][4], "multiple")) echo "[]";?>' data-md-selectize data-md-selectize-bottom   <?php echo $this->FG_TABLE_ADITION[$i][4]?> >
    <?php
    ...
    <?php       
      ...
      <TABLE cellspacing="0"  style="width:80%;margin-top:10px">
        <tr>
            <td width="50%" class="text_azul" style="text-align: right;"><span class="tableBodyRight"><?php echo $this->FG_BUTTON_ADITION_BOTTOM_TEXT?></span></td>
                ...
                <a href="#" onClick="javascript:document.getElementById('myForm').submit();" title="<?php echo gettext("Create a new ");?><?php echo $this->FG_INSTANCE_NAME?>" alt="<?php echo gettext("Create a new ");?> <?php echo $this->FG_INSTANCE_NAME?>" border=0 hspace=0 id="submit4" name="submit2" class="btn btn-primary ">
