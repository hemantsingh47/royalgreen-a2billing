...
<FORM action=<?php echo filter_input(INPUT_SERVER, 'PHP_SELF', FILTER_SANITIZE_URL)?> id="myForm" method="post" name="myForm" class="kt-form">
	...
	<table cellspacing="2"  class="table table-bordered" style="margin:0px 30px 30px 30px; width:95%">
    <tr>
        <td>
            <table cellspacing=0 class="table table-bordered">
                ...
                <tr>
                    <td align=center class="bgcolor_006">
						...
						<INPUT title="Delete this record" alt="Delete this Record" hspace=2 id=submit22 name=submit22 type="submit" value="Confirm Delete" class="btn btn-primary">
                    </td>
                </tr>
                <tr height="5px">
                    <td class="bgcolor_006">&nbsp;</td>
...
{
?>
<!--<table cellspacing="3" class="table table-bordered" style="margin:0px 30px 30px 30px; width:95%">-->
    ...
    <div class="kt-portlet">
       <div class="kt-portlet__head">
				<div class="kt-portlet__head-label">
					 <h3 class="heading_b uk-margin-bottom" style="color: #646c9a;"> <?php echo $this->FG_INTRO_TEXT_ASK_DELETION?></h3>
				</div>
			</div>
...
<FORM action=<?php echo filter_input(INPUT_SERVER, 'PHP_SELF', FILTER_SANITIZE_URL)?> id="myForm" method="post" name="myForm" class="kt-form">
	...
	<!--<table cellspacing="2" class="table table-bordered" style="margin-top:0px; margin-left:30px; margin-right:30px; margin-bottom:30px; width:95%">-->
	
	<table cellspacing="3" class="table table-bordered">
		<?php for($i=0;$i<$this->FG_NB_TABLE_EDITION;$i++){ ?>
		<TR>
		<td>&nbsp;</td>
			<TD valign="middle" class="form_head" style="background:#ffffff;">
				<?php echo $this->FG_TABLE_EDITION[$i][0]?>
			</TD>
			
			
			<TD valign="top" class="tablestyle_001" style="background:#ffffff;">
				<?php
				...
				?>
					<INPUT class="form-control" readonly name=<?php echo $this->FG_TABLE_EDITION[$i][1]?>  <?php echo $this->FG_TABLE_EDITION[$i][4]?> value="<?php echo stripslashes($list[0][$i])?>">
				<?php
					}elseif (strtoupper ($this->FG_TABLE_EDITION[$i][3])==("TEXTAREA")){
				?>
					<TEXTAREA class="form-control" readonly name=<?php echo $this->FG_TABLE_EDITION[$i][1]?> <?php echo $this->FG_TABLE_EDITION[$i][4]?>><?php echo stripslashes($list[0][$i])?></textarea>
				<?php
					...
					?>
					<SELECT class="form-control" disabled name=<?php echo $this->FG_TABLE_EDITION[$i][1]?>>
						<?php
	...
	<TABLE cellspacing="0" class="table table-bordered">
		...
		<tr>
		<td>&nbsp;</td>
		  <td width="50%" class="text_azul" style="background:#ffffff;"><span class="tableBodyRight"><?php echo $this->FG_BUTTON_DELETION_BOTTOM_TEXT?></span></td>
		  <td width="50%" align="right" class="text" style="background:#ffffff;">

				<a href="#" onClick="sendto('delete');"  class="btn btn-danger"><i class="fa flaticon-delete"></i>
