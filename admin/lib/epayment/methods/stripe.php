<?php
include(dirname(__FILE__).'/../includes/methods/stripe.php');

class stripe
{
    public $code, $title, $description, $enabled, $mode;
    public $stripe_allowed_currencies = array('CAD', 'EUR', 'GBP', 'JPY', 'USD', 'MXN', 'AUD', 'NZD', 'BRL');
    ...
    public function stripe()
    {
        ...
        $this->mode = ((STRIPE_PAYMENT_MODE == 'TEST') ? test : live);
	//$this->enabled = true;

        $this->form_action_url = "https://www.admin.easykall.co.uk/customer/result.php";
    }
    ...
    {
        $my_currency = MODULE_PAYMENT_STRIPE_CURRENCY;
        $base_currency = strtoupper($GLOBALS['A2B']->config['global']['base_currency']);
        if ($my_currency =='Selected Currency' && in_array($base_currency, $this->stripe_allowed_currencies) ) {
            $my_currency = $base_currency;
        } elseif (!in_array($my_currency, $this->stripe_allowed_currencies)) {
            $my_currency = 'GBP';
        }
    ...
    {
        return array('MODULE_PAYMENT_STRIPE_STATUS', 'MODULE_PAYMENT_STRIPE_ID', 'STRIPE_PAYMENT_MODE');
    }
}
