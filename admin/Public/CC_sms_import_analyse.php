...
if ($task == 'upload') {
    ...
    while (!feof($fp)) {
        ...
        if (substr($ligne, 0, 1) != '#' && $val[2] != '' && strlen($val[2]) > 0) {
            $FG_ADITION_SECOND_ADD_TABLE = 'cc_sms_rate';
            $FG_ADITION_SECOND_ADD_FIELDS = 'countrycode, countryprefix, countryname'; //$fieldtoimport_sql
...
<table width="95%" border="0" cellspacing="2" align="center" class="records">
  <form name="myform" enctype="multipart/form-data" action="CC_sms_import_analyse.php" method="post" >
