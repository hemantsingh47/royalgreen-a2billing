    ...
    <FORM action=<?php echo filter_input(INPUT_SERVER, 'PHP_SELF', FILTER_SANITIZE_URL); ?> id="myForm" method="post" name="myForm" class="kt-form">
        <div class="kt-portlet__head">
			<div class="kt-portlet__head-label">
				<INPUT type="hidden" name="form_action" value="add" class="form-control">
				<INPUT type="hidden" name="wh" value="<?php echo $wh; ?>" class="form-control">
				<?php
				...
				?>
				<?php //print_r($atmenu);?>
				<INPUT type="hidden" name="atmenu" class="form-control" value="<?php echo $atmenu?>">
				<INPUT type="hidden" name="atmenu" class="form-control" value="<?php echo $atmenu?>">
			</div>
		</div>
...
<div class="widget-content">
        ...
        <?php
            ...
            for ($i=0;$i<$this->FG_NB_TABLE_ADITION;$i++) 
            {
                ...
                if (strlen($this->FG_TABLE_ADITION[$i][16])>1 && strtoupper ($this->FG_TABLE_ADITION[$i][3])!=("HAS_MANY")) 
                {
                    ...
                    <div class="kt-portlet" style="border: 0px solid; padding:10px;">
           ...
           <?php if (!$this-> FG_fit_expression[$i]  &&  isset($this-> FG_fit_expression[$i]) ) 
            ...
            ?>
                ...
                <label class="col-8 col-form-label"  for="<?php echo $this->FG_TABLE_ADITION[$i][1]?>"  <?php echo ucfirst($this->FG_TABLE_ADITION[$i][4])?>><?php echo  $this->FG_TABLE_ADITION[$i][0]?></label>
            <?php 
            } 
            else
            { 
        ...
        ?>
            ...
            <INPUT type="text" style="width:98%" class="form-control" name=<?php echo $this->FG_TABLE_ADITION[$i][1]?>  <?php echo $this->FG_TABLE_ADITION[$i][4]?> value="<?php
    ...
    <!--CAPTCHA IMAGE CODE END HERE-->
        ...
        ?>
               ...
               <SELECT class="form-control"   name='<?php echo $this->FG_TABLE_ADITION[$i][1]?><?php if (strpos($this->FG_TABLE_ADITION[$i][4], "multiple")) echo "[]";?>' <?php if (strpos($this->FG_TABLE_ADITION[$i][4], "multiple")){ echo ""; }else { echo ""; } ?>    <?php echo $this->FG_TABLE_ADITION[$i][4]?> >
        <?php
       ...
       <div class="kt-portlet__foot">
            <div class="kt-form__actions">
                <div class="row">
                        ...
                        <span class="form-text text-muted"><a href="#" onClick="javascript:document.getElementById('myForm').submit();" title="<?php echo gettext("Create a new ");?><?php echo $this->FG_INSTANCE_NAME?>" alt="<?php echo gettext("Create a new ");?> <?php echo $this->FG_INSTANCE_NAME?>" border=0 hspace=0 id="submit4" name="submit2" class="btn btn-primary btn-small ">
                <?php echo $this->FG_ADD_PAGE_CONFIRM_BUTTON; ?> </a></span>
